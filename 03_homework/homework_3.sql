...
COUNT(*) Cnt, SUM(p.quantity * p.cost_to_customer_per_qty) TotalSpent
FROM customer c JOIN customer_purchases p ON c.customer_id = p.customer_id
...
and year are! */
SELECT *, strftime('%m', market_date) Mth, strftime('%Y', market_date) Yr FROM customer_purchases
...
SELECT customer_id, Sum(quantity * cost_to_customer_per_qty) TotalSpent
